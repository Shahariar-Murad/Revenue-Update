...
def load_shared_order_list(file_content, filename):
    ...
    if filename.lower().endswith(".csv"):
        df = pd.read_csv(io.BytesIO(file_content), parse_dates=["Updated At", "Created At"])
    else:
        df = pd.read_excel(io.BytesIO(file_content), parse_dates=["Updated At", "Created At"])
    return df

def get_order_list_for_gateway(df_order_all, gateway_name, id_col="Transaction ID", remove_blank_id=True):
    """Filter the shared Order-list for a gateway, clean duplicates, and sort by Updated At."""
    required_cols = ["Gateway", "Updated At", "Plan Type", "Grand Total", id_col]
    missing_cols = [col for col in required_cols if col not in df_order_all.columns]
    if missing_cols:
        st.error(f"Combined Order-list is missing required column(s): {', '.join(missing_cols)}")
        st.stop()

    df_ord = df_order_all[
        df_order_all["Gateway"].astype(str).str.strip().eq(gateway_name)
    ].copy()

    if df_ord.empty:
        st.error(f"No Order-list rows found for Gateway='{gateway_name}' in the uploaded combined Order-list.")
        st.stop()

    if remove_blank_id:
        mask_blank_id = df_ord[id_col].isna() | (df_ord[id_col].astype(str).str.strip().isin(["", "None", "none", "nan", "NaN"]))
        if mask_blank_id.any():
            st.warning(f"Removed {mask_blank_id.sum()} blank/None {id_col} rows from {gateway_name} Order-list")
            df_ord = df_ord[~mask_blank_id].copy()

    duplicates = df_ord.duplicated(subset=[id_col], keep=False)
    if duplicates.any():
        st.warning(f"Removed {duplicates.sum()} duplicate {id_col} rows from {gateway_name} Order-list")
        df_ord = df_ord.drop_duplicates(subset=[id_col], keep="first")
...
df_order_all = None
if shared_order_file:
    df_order_all = load_shared_order_list(shared_order_file.getvalue(), shared_order_file.name)
    st.sidebar.success(f"Loaded combined Order-list: {len(df_order_all)} rows")
    if "Gateway" in df_order_all.columns:
        st.sidebar.write("Gateway rows:")
        st.sidebar.dataframe(
            df_order_all["Gateway"].value_counts(dropna=False).rename_axis("Gateway").reset_index(name="Rows"),
...
with tab_zen:
    ...
    if zen_file and df_order_all is not None:
        ...
        # Use the shared combined Order-list and filter only Zen Pay rows
        ...
        st.info("Please upload the ZEN Settlement file and the shared combined Order-list file from the sidebar.")
...
with tab_bp:
    ...
    if bp_file and df_order_all is not None:
        # Load BridgerPay
        ...
        # Use the shared combined Order-list and filter only Bridger Pay rows
        ...
        st.info("Please upload the BridgerPay file and the shared combined Order-list file from the sidebar.")
...
with tab_coins:
    ...
    if coins_file and df_order_all is not None:
        # Load Coins Buy
        ...
        # Use the shared combined Order-list and filter only Crypto rows for Coins Buy
        st.subheader("Step 3: Process Coins Buy Order List from Shared File")
        ...
        st.info("Please upload the Coins Buy file and the shared combined Order-list file from the sidebar.")
...
with tab_confirmo:
    ...
    if confirmo_file and df_order_all is not None:
        ...
        # Use the shared combined Order-list and filter only Confirmo rows
